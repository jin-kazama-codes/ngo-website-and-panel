      ...
      <div className={`flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200/80 ${className}`}>
        {langs.map(({ code, label, title }) => (
          <button
            ...
            className={`px-2 py-1 text-xs font-bold rounded transition-all cursor-pointer ${language === code
    ...
    <div className={`flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200 ${className}`}>
      ...
      {langs.map(({ code, label, title }) => (
        <button
          ...
          onClick={() => setLanguage(code)}
          title={title}
          className={`px-2.5 py-1 text-xs font-extrabold rounded-lg transition-all flex items-center gap-1 cursor-pointer ${language === code
