...
export const CommunitiesPage: React.FC<CommunitiesPageProps> = ({ onOpenRegister }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in">
      <div>
        <h1 className="text-3xl font-black text-slate-900">{isHindi ? 'स्थानीय सामुदायिक नेटवर्क' : 'Local Community Network'}</h1>
        <p className="text-sm text-slate-500 mt-1">
          {isHindi ? 'प्रत्येक समुदाय एक विश्वसनीय स्थानीय प्रशासक द्वारा प्रबंधित किया जाता है और हमारे राष्ट्रीय एकजुटता एस्क्रो द्वारा समर्थित है।' : 'Each community is managed by a trusted local administrator and backed by our national solidarity escrow.'}
        </p>
      </div>
              ...
              <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                ...
                <div className="pt-3 border-t border-slate-100 space-y-2 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-500">{isHindi ? 'प्रशासक:' : 'Administrator:'}</span>
                    <span className="font-bold text-slate-800">{comm.adminName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">{isHindi ? 'सक्रिय सदस्य:' : 'Active Members:'}</span>
                    <span className="font-bold text-slate-900">{comm.totalMembers.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">{isHindi ? 'कुल वितरित धनराशि:' : 'Total Funds Disbursed:'}</span>
                    <span className="font-bold text-emerald-700">₹{comm.totalRaisedINR.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">{isHindi ? 'सामुदायिक स्वास्थ्य:' : 'Community Health:'}</span>
                    <span className="font-bold text-emerald-600">{comm.healthScore}% {isHindi ? 'ग्रेड ए' : 'Grade A'}</span>
                  </div>
                </div>
                ...
                >
                  <Users className="w-4 h-4 text-emerald-400" /> {isHindi ? 'समुदाय से जुड़ें (₹50)' : 'Join Community (₹50)'}
                </button>
              </div>
            </div>
          ))}
      ...
      {!loading && communities.length > 0 && currentPage < totalPages && (
