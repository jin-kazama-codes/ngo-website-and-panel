  ...
  return (
    ...
    >
      ...
      <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
        <p className="text-xs leading-relaxed" style={{ color: 'var(--mfct-text-muted)' }}>{displayDescription}</p>
        ...
        <div className="pt-3 space-y-2 text-xs" style={{ borderTop: '1px solid var(--mfct-border)' }}>
          <div className="flex justify-between">
            ...
            <span className="font-bold" style={{ color: 'var(--mfct-dark-green)' }}>{displayAdminName}</span>
          </div>
          <div className="flex justify-between">
            <span style={{ color: 'var(--mfct-text-muted)' }}>{t('communities.active_members', 'Active Members')}:</span>
            <span className="font-bold" style={{ color: 'var(--mfct-dark-green)' }}>{rawComm.totalMembers.toLocaleString('en-IN')}</span>
          </div>
          <div className="flex justify-between">
            <span style={{ color: 'var(--mfct-text-muted)' }}>{t('communities.raised', 'Total Raised')}:</span>
            <span className="font-bold" style={{ color: 'var(--mfct-dark-green)' }}>₹{rawComm.totalRaisedINR.toLocaleString('en-IN')}</span>
          </div>
          <div className="flex justify-between">
            <span style={{ color: 'var(--mfct-text-muted)' }}>{t('communities.health_score', 'Health Score')}:</span>
            <span className="font-bold" style={{ color: 'var(--mfct-gold-dark)' }}>{rawComm.healthScore}% {t('communities.grade_a', 'Grade A')}</span>
          </div>
        </div>
