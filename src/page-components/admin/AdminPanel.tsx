...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const getAllowedRoles = () => {
    ...
    if (userRole === 'executive') {
      return [{ id: 'executive', label: '🛡️ Executive Admin' }];
    }
  ...
  return (
    <div className="h-screen max-h-screen overflow-hidden bg-slate-900 text-slate-100 flex flex-col font-sans selection:bg-emerald-500 selection:text-white">
      {/* Admin Panel Container */}
      <div className="flex flex-1 min-h-0 overflow-hidden relative">
        {/* SIDEBAR */}
        <aside
          className={`bg-slate-950 text-slate-300 w-72 border-r border-slate-800 flex flex-col shrink-0 transition-all duration-300 fixed lg:sticky lg:top-0 h-full max-h-screen inset-y-0 left-0 z-50 ${
        ...
        >
          ...
          <div className={`p-3 py-2.5 border-b border-slate-800/80 bg-slate-900/60 ${!sidebarOpen ? 'lg:hidden' : 'block'}`}>
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block mb-1">
              Active Logged-in Role:
            </span>
            <div className="w-full bg-slate-950 border border-slate-800 text-white rounded-xl px-3 py-2 text-xs font-bold flex items-center gap-2 select-none shadow-inner">
              {roleBadges[currentRole].icon}
              <span className="text-white font-extrabold">{roleBadges[currentRole].label}</span>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-bold text-slate-400 hover:text-white hover:bg-slate-900 transition-all"
                >
          ...
          <div className="p-2.5 border-t border-slate-800 space-y-1.5">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-emerald-400 font-bold text-xs border border-slate-800 transition-all"
            >
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-rose-950/60 hover:bg-rose-900/80 text-rose-300 font-bold text-xs border border-rose-900/50 transition-all cursor-pointer"
              >
            ...
            <div className={`p-2 rounded-xl bg-slate-900 border border-slate-800 flex items-center gap-2.5 ${!sidebarOpen ? 'lg:hidden' : 'flex'}`}>
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full min-h-0 bg-slate-950 overflow-y-auto">
          {/* Topbar Header */}
          <header className="bg-slate-900 border-b border-slate-800 py-3.5 px-4 sm:px-6 sticky top-0 z-30 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  className="py-1.5 px-3 rounded-xl bg-rose-950/80 hover:bg-rose-900 text-rose-200 border border-rose-800/60 font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                    ...
                    onSubmit={async (e) => {
                      ...
                      try {
                        await submitEmergencyAidRequest({
                          ...
                          communityName: activeUser.communityName,
                        ...
                        console.error(err);
                        alert('Failed to submit aid request. Please try again.');
                      }
                    }}
